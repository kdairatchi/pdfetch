...
# Check if domain is provided or fetch a random one
if [ -z "$1" ]; then
...
mkdir -p "$RAW_DIR" "$PII_DIR" "$SCREENSHOTS_DIR"
...
cat "$RAW_DIR/urls.txt" | xargs -P10 -I{} sh -c '
    ...
    # Take a screenshot of the URL if it's an HTML page
    if [[ "$FILETYPE" == "text/html" ]]; then
        SCREENSHOT="'$SCREENSHOTS_DIR'/$(basename "$URL").pdf"
        wkhtmltopdf "$URL" "$SCREENSHOT" > /dev/null 2>&1
